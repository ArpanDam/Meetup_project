...
for i in range(2088): # iterate for each group
    ...
    os.chdir('./m2_number_of_event')
    file_name2="m4_entropy"+" "+str(i)
    
    entropy_file=pickle.load(open(file_name2,"rb")) # for a particular group
            ...
            if(neighbours[member_id][j]==0):  
                ...
                try:
                    if(neighbours[member_id][j][0] == 0):
                        ...
                        avg=0
                        for k in range(len(neighbours[member_id][j])): # iterate for each neighbour of member_id
                            neighbour_id=neighbours[member_id][j][k]
                            #the window is j
                            # need to find degree of the member neighbour_id at window j
                            sum1=sum1+entropy_file[neighbour_id][j]
                    ...
                    list1.append(0) 
        dict1[key]=list1
    file_name="m11_avg_entropy_1_hop"+" "+str(i)
